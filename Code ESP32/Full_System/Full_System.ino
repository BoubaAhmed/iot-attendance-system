...
bool checkSchedule() {
    ...
    String schedulePath = "/schedule/" + currentRoom + "/" + currentDay;
    
    if (!Firebase.getJSON(fbData, schedulePath)) {
        Serial.println("Pas de cours aujourd'hui");
        sessionActive = false;
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (currentMinutes >= startMinutes && currentMinutes < endMinutes) {
            ...
            String sessionPath = schedulePath + "/" + key;
            ...
            currentSessionId = getCurrentDate() + "_" + currentRoom + "_" + startTime;
            
            // Créer ou vérifier la session
            createSession(startTime, endTime);
            
            foundSession = true;
            sessionActive = true;
            
            String subjectName = currentSubject;
            // Récupérer le nom complet de la matière
    ...
    if (!foundSession) {
        Serial.println("Pas de cours actuellement");
        sessionActive = false;
